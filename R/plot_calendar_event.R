...
suppressMessages({library(did); library(fixest); library(dplyr); library(ggplot2); library(cowplot)})
...
p_cs <- ggplot(cs_df, aes(time, estimate)) +
  ...
  labs(x = "Calendar year", y = "ATT(t)") + base
...
p_sa <- ggplot(sa_df, aes(time, estimate)) +
  ...
  annotate("text", x = max(sa_df$time), y = sa_overall, vjust = -0.6, hjust = 1,
           ...
           size = 3, fontface = "bold") +
  labs(x = "Event time (years relative to treatment)", y = "ATT(e)") + base

p <- cowplot::plot_grid(p_cs, p_sa, ncol = 1)
